# ==========================================
# AŞAMA 5: BÜYÜK BİRLEŞİK (GRAND UNIFIED) OPTİMİZASYON
# ==========================================
# DİKKAT: Bu kod Colab üzerinde yaklaşık 45 - 90 dakika arası sürebilir.
# Hiçbir parametre atlanmamış, zaman sınırı tamamen kaldırılmıştır.
# !pip install optuna statsmodels pandas_ta
...
tf.get_logger().setLevel('ERROR') # Gereksiz TF loglarını sustur
...
# Dinamik Veri Üretici (Optuna döngüleri içinde şekil değiştiren tensörler)
...
def create_objective(min_pencere, max_pencere, is_short, loss_type):
    def objective(trial):
        ...
        # Cross validation veri boyutu: son 1000 gün (Optimizasyonun çıldırmaması için)
        tr_len = len(x_tr)
...
kisa_model.fit(x_tr_k, y_tr_k, batch_size=32, epochs=40, verbose=0)
...
lstm_out2 = LSTM(16, return_sequences=False)(attention_out) # Attention daraltması
...
orta_model.fit(x_tr_o, y_tr_o, batch_size=32, epochs=30, verbose=0)
...
plt.title('Büyük Birleşik Optimizasyon: Tamamen Otonom Şampiyon', fontsize=16)

plt.plot(y_test_gercek, label='Gerçek AAPL Fiyatı', color='black', linewidth=2)
plt.plot(ensemble_preds, label='MUTLAK OTONOM TAHMİN', color='red', linewidth=2.5)
...
print("\n[BAŞARILI] Büyük Birleşik Optimizasyon tamamlandı! Elde edilen bu RMSE makinenin sınırıdır.")
